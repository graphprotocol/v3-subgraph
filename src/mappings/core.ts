...
import { Bundle, Burn, Factory, Mint, Pool, PoolHourData, Swap, Tick, Token } from '../types/schema'
...
class PoolData {
  static add(timestamp: i32, pool: Pool, volumeToken0: BigDecimal, volumeToken1: BigDecimal, volumeUSD: BigDecimal, feesUSD: BigDecimal) {
...
export class TickData {
  ...
  static add(timestamp: i32, tick: Tick, pool: Pool | string) {
    assert(false, "not implemented")
  }
}
...
export function handleInitialize(event: Initialize): void {
  ...
  PoolData.add(timestamp, pool, ZERO_BD, ZERO_BD, ZERO_BD, ZERO_BD)
...
export function handleMint(event: MintEvent): void {
  ...
  UniswapData.add(timestamp, ZERO_BD, ZERO_BD, ZERO_BD, factory.totalValueLockedUSD)
  PoolData.add(timestamp, pool, ZERO_BD, ZERO_BD, ZERO_BD, ZERO_BD)
  updateTokenData(token0 as Token, event, ZERO_BD, ZERO_BD, ZERO_BD)
  updateTokenData(token1 as Token, event, ZERO_BD, ZERO_BD, ZERO_BD)
...
export function handleSwap(event: SwapEvent): void {
  ...
  // interval data
  let timestamp = event.block.timestamp.toI32();
  UniswapData.add(timestamp, amountTotalETHTracked, amountTotalUSDTracked, feesUSD, factory.totalValueLockedUSD)
  PoolData.add(timestamp, pool, amount0Abs, amount1Abs, amountTotalUSDTracked, feesUSD)
...
function updateTickFeeVarsAndSave(tick: Tick, event: ethereum.Event): void {
  ...
  TickData.add(event.block.timestamp.toI32(), tick, tick.pool)
}
